    ...
    visitor: {
      ...
      AssignmentExpression: function AssignmentExpression(path) {
        if (path && path.node && path.node.right && t.isNewExpression(path.node.right)) {
          ...
          try {
            var objName = path.node.left.object.name;
            var propName = path.node.left.property.name;
            for (var item in args) {
              var argItem = args[item];
              if (t.isFunctionExpression(argItem)) {
                var newNode = generateAst(argItem, false, false, objName, propName);
                path.replaceWith(newNode);
              }
            }
          } catch (e) {
